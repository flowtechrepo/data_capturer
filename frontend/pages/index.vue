<template lang='pug'>
  .shortUrlContainer
    .flowContainer
      .left-box
        el-menu.el-menu-vertical-demo(default-active="/" @select="changeurl" style="height: 100%; width: 100%")
          el-menu-item(index="/")
            i.el-icon-menu
      ...
      .right-box
        el-dialog(title="Add Url" :visible.sync="dialogVisible")
          .addBox
            el-form
              el-form-item(label="Pages")
                el-select.full-width(v-model="url.page" filterable allow-create default-first-option)
        ...
        .addBtn
          el-input.addBtnLeft(placeholder="input to filter..." v-model="search" @input="doSearch")
          el-button.addBtnRight(@click="dialogVisible  = true") Batch Add Url
        .shortUrlList
          el-table.full-width(:data="urls" border stripe empty-text="No urls")
            el-table-column(type="index")
            el-table-column(label="Code" prop="shortPath" width="120px")
            el-table-column(label="Short Url" prop="shortUrl" width="200px")
            el-table-column(label="Full Url" prop="fullPath")
            //- el-table-column(label="Owner Id" prop="id_owner" width="80px")
            //- el-table-column(label="Deal Id" prop="id_deal" width="80px")
            //- el-table-column(label="Client Id" prop="id_client" width="80px")
            //- el-table-column(label="Client Name" prop="client_name" width="80px")
            //- el-table-column(label="Phone No" prop="phone_no" width="80px")
            el-table-column(label="Lang" prop="language" width="80px")
            el-table-column(label="Channel" prop="id_channel" width="80px")
            el-table-column(label="Actions" width="120px")
              template(slot-scope="scope")
                el-button(@click="visit(scope.row.shortUrl)") Visit
</template>
...
export default {
  ...
  methods: {
    ...
    doSearch () {
      this.urls = this.urls.filter(item => JSON.stringify(item).indexOf(this.search) !== -1)
    },
    async initUrl () {
